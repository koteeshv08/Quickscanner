...
def test_wordlist(url,wordlist,headers):
    ...
    for line in file_pointer_lfi:
        ...
        try:
            ...
            continue
        if(checkPayload(res)):
            print(colored("[+] LOCAL FILE INCLUSION VULNERABILITY FOUND LINK        -->  " + u,'red',attrs=['bold']))
            local_file_inclusion_list.append(url)
            count+=1
...
def test_php_data(url,headers):
    ...
    for i in range(len(testL)):
        ...
        try:
            ...
            continue
        if(checkPayload(res)):
            print(colored("[+] LOCAL FILE INCLUSION VULNERABILITY FOUND LINK (RCE)  -->  " + u,'red',attrs=['bold']))
            local_file_inclusion_list.append(url)
            count+=1
            break
    #Windows
    ...
    for i in range(len(testL)):
        ...
        for j in range(len(posts)):
            try:
                ...
                continue
            if(checkPayload(res)):
                print(colored("[+] LOCAL FILE INCLUSION VULNERABILITY FOUND LINK (RCE)  -->  " + u+ " -> HTTP POST: " + posts[j],'red',attrs=['bold']))
                local_file_inclusion_list.append(url)
                count+=1
                break
        #Windows
        for k in range(len(testW)):
            ...
            for l in range(len(posts)):
                try:
                    ...
                    continue
                if(checkPayload(res)):
                    print(colored("[+] LOCAL FILE INCLUSION VULNERABILITY FOUND LINK (RCE)  -->  " + u+ " -> HTTP POST: " + posts[l],'red',attrs=['bold']))
                    local_file_inclusion_list.append(url)
                    count+=1
                    break
...
def main(url,headers):
    try: 
        ...
        test_php_filter(url,headers)
        test_php_input(url,headers)
        test_php_data(url,headers)
        test_php_expect(url,headers)
        test_wordlist(url,"payloads/local_file_inclusion.txt",headers)
